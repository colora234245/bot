...
client.on("userUpdate", async function(eskiii, yeniii) {
  const guildID = "909201710906499083"
  const roleID = "1004894589087395890"//taglı_rol
  const tag = "?"
  const log2 = '1005172102233587802'
  ...
  if (yeniii.username !== eskiii.username) {
      ...
      if (eskiii.username.includes(tag) && !yeniii.username.includes(tag)) {
        if (yeniii.discriminator === "2020") return client.channels.cache.get(log2).send(`${yeniii} İsim tagımızı bıraktı ama hala üzerinde etiket olduğu için \`Tagges\` rolünü almadım`);
 ...
 if (yeniii.discriminator !== eskiii.discriminator) {
      if (eskiii.discriminator == "2020" && yeniii.discriminator !== "2020") {
      ...
      } else if (eskiii.discriminator !== "2020" && yeniii.discriminator == "2020") {
          member.roles.add(roleID)
...
client.on("guildMemberAdd", member => {
  ...
  if (member.user.discriminator === "2020") {
    setTimeout(function(){  
   client.channels.cache.get(log2).send(`${member} sunucuya \`2020\` etiketiyle geldiği için \`tagges\` rolünü verdim.`)
  }, 2000);
  setTimeout(function(){  
   member.roles.add(role)
  }, 5000);
